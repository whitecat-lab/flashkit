...
struct PfSenseProfile: VendorImageProfile {
    ...
    func match(in context: ImageClassificationContext) -> VendorProfileMatch? {
        let filenameHit = context.filenameContainsAny(["pfsense"])
        let volumeHit = context.volumeContainsAny(["pfsense"])
        ...
        case .rawDiskImage, .compressedRawDiskImage:
            let warnings = isMemstick ? [] : ["pfSense was detected, but the filename did not clearly identify a memstick-style image."]
        ...
        case .plainISO, .hybridISO:
            return VendorProfileMatch(
                ...
                warnings: ["pfSense media was detected as an ISO-like image instead of a memstick image. Review whether a raw memstick build would be safer for USB media."],
        ...
        case .unknownAmbiguous:
            return VendorProfileMatch(
                ...
                warnings: ["The file looks pfSense-related, but the backend could not classify a clean memstick or ISO container."],
